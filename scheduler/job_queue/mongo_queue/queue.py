...
class Queue:
    ...
    def ensure_indexes(self):
        ...
        self.collection.create_indexes([next_index, update_index, unique_index, update_index2, next_index2])
    ...
    def repair(self):
        ...
        self.collection.find_one_and_update(
            filter={
                "locked_by": {"$ne": None},
                "locked_at": {
                    "$lt": datetime.utcnow() - timedelta(seconds=self.timeout)}},
            update={
    ...
    def next(self, channel="default"):
        aggregate_result = list(self.collection.aggregate([
            {'$match': {'locked_by': None, 'locked_at': None,
                        ...
                        "$or": [{"run_after": { "$exists": False }}, { "run_after" : {"$lt": datetime.utcnow()}}]
            }},
            # {"$lookup":
            #     {
