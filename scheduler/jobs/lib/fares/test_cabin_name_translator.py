...
class ConverterOneWayTest(unittest.TestCase):
    ...
    #     fares = data['fares']
    #     convertedFares = converter.convert(data['type'], itineraries, fares)
    #     self.assertIsNotNone(convertedFares)
    #     self.assertEqual(len(convertedFares),4)
    ...
    def test_economy_business_fares(self):
        ...
        fares = [
            ...
            {"scrapedFrom":"flypgs.com",            "fareAmount": 200, "fareCurrency": "USD", "itinCabins": [{"cabin": ["ECO"],     "itinId": 0, "cabinClass": "Y"}]}
        ]
        for fare in fares:
            converted = converter.convert_single_fare(itineraries, fare)
            self.assertEqual('ECONOMY', converted['cabinName'])
